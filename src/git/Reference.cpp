...
Reference::operator git_reference *() const
{
  return d.data();
}
...
{
  return git_reference_owner(d.data());
}

bool Reference::isTag() const
{
  return git_reference_is_tag(d.data());
}
...
bool Reference::isDetachedHead() const
{
  return (qualifiedName() == "HEAD");
}
