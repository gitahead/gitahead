    ...
    mConfig(config)
{
    ...
    connect(this, &QToolButton::triggered, this, &TemplateButton::actionTriggered);
}
...
{
    /*
     * Workaround, because reading all variables in one backend is not possible with
     * this library.
     * Cleaner would be to have:
     * [Templates]
     *  T1 = value of template 1
     *  T2 = value of template 2
     *
    ...
    for (auto templ: mTemplates) {
        ...
        value = value.replace("\n", "\\n");
        value = value.replace("\t", "\\t");
        mConfig.setValue(kTemplatesKey, templ.name + separator + ".*", templ.name + separator + value);
    }
}
