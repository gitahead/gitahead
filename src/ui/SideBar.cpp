...
public:
  ...
  QModelIndex parent(const QModelIndex &index) const override
  {
    ...
    if (qobject_cast<RepoView *>(ptr) ||
        qobject_cast<TabWidget *>(ptr))
    ...
    if (qobject_cast<Account *>(ptr) ||
        qobject_cast<Accounts *>(ptr))
    ...
    int role = Qt::DisplayRole) const override
  {
    ...
    switch (role) {
      case Qt::DisplayRole: {
        if (!parent.isValid()) {
          switch (row) {
            case Repo:   return tr("open");
            case Recent: return tr("recent");
            case Remote: return tr("remote");
            default:     return QVariant();
          }
        }
      ...
      case Qt::ForegroundRole:
        return QPalette().brush(
          !parent.isValid() ? QPalette::BrightText : QPalette::Text);
